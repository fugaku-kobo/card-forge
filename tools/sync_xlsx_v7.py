...
PAGE_FIX = {
    ...
    'PG-002': '全キャラの攻撃力を「3」にする。その後に適用されるバフ・デバフのみ加減算する',
    ...
    'PG-013': '自分の場のキャラは攻撃力 −2(最低1)',
    'PG-014': '相手の場のキャラは攻撃力 −2(最低1)',
...
# --- 主人公(K章)両面共通効果 ---
HERO_FIX = {
    'BJ-H001': '「執刀」1ターンに1度、キャラ1体を「執刀」する。自分のキャラなら→このターン戦闘で没にならない+攻撃力+2(救命)。相手のキャラなら→このターン、ブロックできない。',
    'HT-H003': '「輪廻」1ターンに1度、メインフェイズの最初に、没案ゾーンの自分のキャラ1体を場に出す(召喚酔いあり・空き枠が必要)。そのキャラは攻撃力+2(永続)。',
    'RK-H004': '「ふたつの心」サファイアのプレイヤーは「心」を持つ(開始時は【王子の心】)。1ターンに1度、心を切り替えられる。【王子の心】=自分のキャラは攻撃する時、攻撃力+2。【お姫さまの心】=自分のキャラはブロックする時、攻撃力+2。',
}
# --- 詳細ルール I章リーダー行(部分一致で行特定→全文置換) ---
DETAIL_LEADER_FIX = {
    '「執刀」': '・ブラック・ジャック「執刀」:1ターンに1度、キャラ1体を執刀。自分のキャラ＝このターン戦闘で没にならない+攻撃力+2(救命)／相手のキャラ＝このターンブロックできない。(コントロール型)',
    '「輪廻」': '・ナギ(火の鳥)「輪廻」:1ターンに1度、メインフェイズの最初に没案ゾーンの自分のキャラ1体を場に出す(召喚酔いあり・空き枠が必要)。そのキャラは攻撃力+2(永続)。(蘇生型)',
    '「ふたつの心」': '・サファイア(リボンの騎士)「ふたつの心」:「心」を持ち、1ターンに1度【王子の心】⇔【お姫さまの心】を切り替えられる。【王子】＝自分のキャラは攻撃時に攻撃力+2／【お姫さま】＝自分のキャラはブロック時に攻撃力+2。(変身・適応型)',
}
# --- 詳細ルール末尾に追記する補足ルール ---
DETAIL_APPEND = [
    '',
    ...
    '　- 盾持ち・守り手は代替判定でも有効(攻撃力・ダイスに関係なく攻撃を完全に止める)。',
